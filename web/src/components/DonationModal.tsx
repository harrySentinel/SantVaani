...
const DonationModal: React.FC<DonationModalProps> = ({ organization, onClose }) => {
  ...
  const upiId = organization.upi_id || 'santvaani@paytm';

  // Generate UPI QR code URL using Google Charts API
  const generateQRCode = (upiId: string, amount?: number) => {
    const upiString = amount
      ? `upi://pay?pa=${upiId}&pn=${encodeURIComponent(organization.organization_name)}&am=${amount}&cu=INR&tn=Donation for ${encodeURIComponent(organization.organization_name)}`
      : `upi://pay?pa=${upiId}&pn=${encodeURIComponent(organization.organization_name)}&cu=INR&tn=Donation for ${encodeURIComponent(organization.organization_name)}`;
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-sm sm:max-w-md mx-auto bg-white rounded-2xl shadow-2xl border-0 p-0 overflow-hidden max-h-[90vh] overflow-y-auto">
        <DialogHeader className="sr-only">
          <DialogTitle>Support Donation for {organization.organization_name}</DialogTitle>
          <DialogDescription>
            Make a donation to support {organization.organization_name} through UPI payment
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-4 sm:p-6 pb-4">
          ...
          <div className="mt-2">
            <h3 className="font-semibold text-base sm:text-lg">{organization.organization_name}</h3>
            <p className="text-orange-100 text-xs sm:text-sm">{organization.organization_name_hi}</p>
            <p className="text-orange-100 text-xs sm:text-sm">{`${organization.city}, ${organization.state}`}</p>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="text-center">
            <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-4 sm:p-8 border border-orange-200">
              <div className="w-40 h-40 sm:w-48 sm:h-48 mx-auto bg-white rounded-xl shadow-lg flex items-center justify-center border-2 border-orange-200 overflow-hidden">
                <img 
                  src={qrCodeUrl} 
                  alt={`UPI QR Code for ${organization.organization_name}`}
                  ...
                  onError={(e) => {
                    ...
                    fallback.innerHTML = `
                      <div class="text-center p-4">
                        ...
                        <p class="text-xs text-gray-700 font-medium">${organization.organization_name}</p>
                      </div>
                    `;
          ...
          <div className="bg-gradient-to-r from-blue-50 to-green-50 rounded-xl p-3 sm:p-4 border border-gray-200">
            ...
            <p className="text-xs sm:text-sm text-gray-600">
              <strong>Established:</strong> {organization.established_year}
            </p>
          </div>
