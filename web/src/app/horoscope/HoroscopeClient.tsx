...
export default function HoroscopeClient() {
  ...
  useEffect(() => {
    const fetchZodiacSigns = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/horoscope/zodiac/list`);
  ...
  const fetchHoroscope = async (signId: string, period: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/horoscope/${signId}/${period}`);
      const data = await response.json();
  ...
  const summarizeHoroscope = async (period: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/horoscope/summarize`, {
        method: 'POST',
