...
import { ChevronLeft, ChevronRight, ArrowRight, Check, Minus, Plus, X } from 'lucide-react';
...
import { stripChapterPrefix, type Chapter, type BookInfo } from '@/lib/leelaayen';
...
export default function ChapterReader({ chapter, book, nextChapter, prevChapter, totalChapters }: ChapterReaderProps) {
  ...
  useEffect(() => {
    supabase.rpc('increment_chapter_views', { chapter_slug: chapter.slug });
  }, [chapter.slug]);
  ...
  useEffect(() => {
    if (restoredRef.current || !chapterProgress) return;
