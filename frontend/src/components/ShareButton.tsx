...
export default function ShareButton({ 
  ...
  const handleWhatsAppShare = () => {
    try {
      const whatsappMessage = generateSaintWhatsAppMessage(saint);
      shareOnWhatsApp(whatsappMessage);
      toast({
        title: "WhatsApp Share",
        description: "Opening WhatsApp to share saint information",
      });
      setIsOpen(false);
    } catch (error) {
      toast({
    ...
    try {
      ...
      if (success) {
        toast({
          ...
          description: "Full saint information copied to clipboard - ready to paste anywhere!",
        });
      } else {
  ...
  const handleNativeShare = async () => {
    ...
    if (shared) {
      toast({
        ...
        description: "Saint information shared",
      });
      setIsOpen(false);
    } else {
  ...
  const handleTwitterShare = () => {
    ...
    toast({
      title: "Twitter Share",
      description: "Opening Twitter to share saint information",
    });
    setIsOpen(false);
  };
  ...
  const handleFacebookShare = () => {
    ...
    toast({
      ...
      description: "Opening Facebook to share saint profile",
    });
    setIsOpen(false);
  };
  ...
  const showManualCopyModal = (textToCopy?: string) => {
    const copyText = textToCopy || decodeURIComponent(generateSaintWhatsAppMessage(saint));
    ...
    modal.innerHTML = `
      <div style="
        ...
        text-align: center;
      ">
        <h3 style="margin-bottom: 15px; color: #333;">Copy Saint Information</h3>
        <textarea 
          rows="6"
          readonly
          style="
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button 
          ...
          size={size}
          className={`${className}`}
          aria-label="Share saint information"
        >
