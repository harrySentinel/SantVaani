...
const Signup = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.username || !formData.email || !formData.password || !formData.confirmPassword) {
      toast({ title: "Missing Information", description: "Please fill in all required fields", duration: 3000 });
      return;
    }
    if (formData.username.length < 3) {
      toast({ title: "Invalid Username", description: "Username must be at least 3 characters", duration: 3000 });
      return;
    }
    if (!/^[a-zA-Z0-9_]+$/.test(formData.username)) {
      toast({ title: "Invalid Username", description: "Letters, numbers, and underscores only", duration: 3000 });
      return;
    }
    if (formData.password !== formData.confirmPassword) {
      toast({ title: "Password mismatch", description: "Passwords do not match", duration: 3000 });
      return;
    }
    if (formData.password.length < 6) {
      toast({ title: "Weak password", description: "Password must be at least 6 characters", duration: 3000 });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Account created", description: `Welcome ${formData.name}! Please check your email to verify.`, duration: 5000 });
      setTimeout(() => navigate('/login'), 2000);
    } catch (error: any) {
      toast({ title: "Signup failed", description: error.message || "Failed to create account. Please try again.", duration: 4000 });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      toast({ title: "Google signup failed", description: error.message || "Please try again.", duration: 4000 });
      setIsLoading(false);
    }
  };

  const passwordStrength = formData.password.length >= 8 ? 'Strong' : formData.password.length >= 6 ? 'Medium' : 'Weak';
  const passwordStrengthColor = formData.password.length >= 8 ? 'bg-green-400' : formData.password.length >= 6 ? 'bg-yellow-400' : 'bg-red-400';
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-b from-orange-50 to-amber-50 flex-col items-center justify-center p-16 space-y-8">
        ...
        <div className="space-y-3 text-center">
          ...
          <p className="text-gray-500 leading-relaxed max-w-xs">
            Thousands of seekers have already found their spiritual home here. Join them.
          </p>
        </div>
        <div className="border-l-4 border-orange-300 pl-5 text-left max-w-xs">
          ...
          <p className="text-orange-500 text-xs mt-1">Surrender to me alone — Gita 18.66</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="space-y-1">
            <Link to="/" className="inline-flex items-center gap-2 text-gray-400 hover:text-gray-600 text-sm transition-colors mb-4">
              ← Back to home
            </Link>
            <h1 className="text-3xl font-bold text-gray-900">Create account</h1>
            <p className="text-gray-500">Join the SantVaani community</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="username" className="text-sm text-gray-700 font-medium">Username *</Label>
                <Input
                  id="username"
                  type="text"
                  ...
                  <div className="flex gap-1 flex-1">
                    {[...Array(3)].map((_, i) => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${
                        (i === 0) || (i === 1 && formData.password.length >= 6) || (i === 2 && formData.password.length >= 8)
                          ? passwordStrengthColor : 'bg-gray-100'
                      }`} />
                    ))}
                  </div>
            ...
            <div className="space-y-1.5">
              ...
              {formData.confirmPassword && (
                <p className={`text-xs ${formData.password === formData.confirmPassword ? 'text-green-500' : 'text-red-400'}`}>
